...
| 12 | Remove under-sourced capability-to-scope entry from breadcrumbs doc | done | 2026-06-19 (doc-only; off `main`, parallel to in-flight Plan 4). Gordon's call: removed former **entry 7** from `docs/design/APPROACH_DESIGN_REASONING.md` — the capability-to-scope law's *broader unification* (control-theory-altitude / hardware-abstraction), the one soft-sourced claim Plan 9's report flagged. Dropped rather than carried with a caveat. Old entry 8 (R7/R8 split) renumbered → entry 7 so headings stay contiguous **1–7**; no other entry's content changed; no dangling references existed (the law was not cross-referenced elsewhere); `Status: reconstructed` note + source list left intact. The capability-to-scope **law itself is untouched** where validly attested (APPROACH.md, METHODOLOGY R7) — only the breadcrumbs doc's under-sourced entry was removed. Plan 9's historical registry row is left as-is (it accurately records Plan 9 landing 8 entries). Also handled in the same instruction: **cancelled** the migration-summary §11 appendix prompt — it was never written (no prompt file, branch, or PR in either repo; `docs/MIGRATION_SUMMARY.md` untouched), so nothing to revert. Boot-audit: highest used = 11, Plan 4 reserved → 12 next contiguous free. Prompt+report at `docs/prompts/plan-12-remove-breadcrumbs-entry7.{md,report.md}`. |
...
**Last Updated:** 2026-06-19 (**Plan 12 — remove under-sourced breadcrumbs entry.** Gordon's call: removed former entry 7 from `docs/design/APPROACH_DESIGN_REASONING.md` — the capability-to-scope law's broader unification (control-theory / hardware-abstraction), the one soft-sourced claim Plan 9 flagged; dropped rather than carried with a caveat. Old entry 8 renumbered → 7 (headings now contiguous 1–7); no other content changed; no dangling refs; `Status: reconstructed` + sources intact. The law itself stays where validly attested (APPROACH.md, METHODOLOGY R7). Same instruction also cancelled the never-written migration-summary §11 appendix prompt (nothing to revert; `docs/MIGRATION_SUMMARY.md` untouched). Boot-audit: highest = 11, Plan 4 reserved → 12 next free. Prompt+report at `docs/prompts/plan-12-remove-breadcrumbs-entry7.{md,report.md}`.)

**Previously:** 2026-06-17 (**Plan 11 — session-starter cold-start block.** Added a new-account cold-start block near the top of `session-starter.md` (both repos) — a conditional section pointing a fresh Claude to the migration trio in read-order (`docs/MIGRATION_SUMMARY.md` → `MIGRATION_CHECKLIST.md` → `PROJECT_MIGRATION_2026-06-07.md`), then to the normal start-each-session flow; a continuing session skips it. Consolidation only (no new standalone doc); block worded identically across both repos (extracted-block `diff` clean), rest of each file untouched. All three docs verified present + tracked before wiring. This repo Plan 11 / sibling oxygen-mvp Plan 51. Supersedes the earlier infra-install runbook prompt per Gordon's consolidation decision. Boot-audit: highest = 10, Plan 4 reserved → 11 next free. Prompt+report at `docs/prompts/plan-11-session-starter-coldstart.{md,report.md}`.)
