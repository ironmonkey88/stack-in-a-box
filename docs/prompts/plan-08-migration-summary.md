# Prompt — Land the project migration summary in both repos

**Kind:** coding
**Date:** 2026-06-17
**Scope:** `oxygen-mvp` and `stack-in-a-box` — new `docs/MIGRATION_SUMMARY.md` at each repo's `docs/` root (byte-identical, like APPROACH.md); pointer wiring in each repo's `session-starter.md`; LOG.md / TASKS.md updates per repo convention. One canonical Markdown body, identical in both repos.
...
`docs/prompts/plan-NN-migration-summary.md` on a new branch
`claude/plan-NN-migration-summary`. This file write is the first commit on the
...
number. Boot-audit the registry first (the registries show reserved-but-unused
slots; do not claim a reserved one). Because this lands in **both** repos, each
repo gets its own plan number from its own registry; note both numbers in the
branch's report. If the next contiguous slot is ambiguous in either repo, halt
and surface before branching.
...
The project gains a single **migration summary** — a cold-start handoff written
so a fresh Claude Chat in a different account can resume the project with
nothing lost. It captures the state that lives *outside* the repos: the working
agreements (Chat/Code split, the paused MCP-commit path, the Phase-0 prompt-
self-write discipline, the delivery-as-downloadable-file rules, the trigger
commands), the in-flight technical state, the deferred decisions and their
reasoning, the repo locations and stack, and a first-session checklist. The
repos remain the source of truth for code and authority documents; this doc is
the source of truth for *how we work* and *where we left off* at migration time.

After this lands, the project's restart instructions are version-controlled
alongside the work they describe, rather than living only in a chat thread that
the account move would strand.

## Context (conditional)

- This doc sits at the **same altitude as APPROACH.md**: a shared artifact meant
  to exist in **both** repos, byte-identical at landing time, kept consistent
  via the same Code-proposes / human-approves posture.
- The canonical body is provided in full below (see "Canonical content"). It was
  authored in Chat; **transcribe it, do not rewrite it.** Minor mechanical fixes
  only (e.g. correcting a repo-relative link target). The body deliberately
  describes both repos, so the same text is correct in each.
- The body contains forward-looking "verify against LOG.md" instructions aimed
  at the *next* Chat — those are intentional and must be transcribed as-is, not
  resolved by you.
- Filename is `MIGRATION_SUMMARY.md` under `docs/` at each repo. (Placing it in
  `docs/` rather than repo root keeps the root reserved for the standing
  authority documents — APPROACH/CLAUDE/PHILOSOPHY/METHODOLOGY/PROMPTS. If you
  believe repo root serves better, surface it as a recommendation — do not move
  it unilaterally.)

## Work (required)

1. **Create `docs/MIGRATION_SUMMARY.md` in `oxygen-mvp`** with the canonical
   body below, verbatim.
2. **Create `docs/MIGRATION_SUMMARY.md` in `stack-in-a-box`** with the identical
   canonical body.
3. **Wire `oxygen-mvp` pointer:** add a `MIGRATION_SUMMARY.md` pointer to
   `session-starter.md` — a one-line entry under the existing "Key Files to
   Know" (or nearest equivalent), framed as "read this if you are restarting the
   project in a new account / fresh context."
4. **Wire `stack-in-a-box` pointer:** the equivalent `session-starter.md` entry
   for that repo's structure. Adapt to its conventions and note the adaptation.
5. **LOG.md + TASKS.md** updates in each repo per that repo's conventions.

## Verification (required)

Static-artifact gates (documentation change; all gates are static):

- `docs/MIGRATION_SUMMARY.md` exists in both repos with **identical** body
  content (verify by `diff` — byte-identical, like APPROACH.md).
- Each repo's `session-starter.md` references the migration summary.
- No existing doc edited beyond the wiring sites named in Work items 3–4.
...
- If the next plan number is ambiguous in either repo's registry, surface and
  ask rather than guessing. (Session 47 in oxygen-mvp caught Chat guessing the
  wrong slot — read the registry, don't infer; respect reserved-but-unused
  slots.)
- If transcribing the body surfaces a factual contradiction with current repo
  state (e.g. the body names an in-flight plan that the registry now shows
  closed), **do not edit the body to "correct" it** — the body is a point-in-
  time snapshot and says so. Surface the discrepancy in the report instead.

## Out of scope (conditional)

- No `.docx` / Google Doc export. The repo holds the Markdown source of truth.
- No edits to any authority document beyond the two `session-starter.md` wiring
  pointers.
...
- Then: `MIGRATION_SUMMARY.md` creation, then the wiring edit, then LOG/TASKS —
  committed per Work item (documentation-only; commits without a gate).
- Commit messages name the plan number.
- One PR per repo. Open the PR when the first commit lands; merge when
  Verification passes.
- Write the report to the sibling
  `docs/prompts/plan-NN-migration-summary.report.md` before merge, in each repo.
  Report both repos' plan numbers and PR links.

---

## Canonical content — transcribe verbatim into `docs/MIGRATION_SUMMARY.md` (both repos)

> Everything between the rule below and the end of this prompt is the literal
> file body. Begin the file at the `# Project Migration Summary` heading.

---

<!-- BEGIN MIGRATION_SUMMARY.md BODY -->
