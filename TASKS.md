...
**Plan 2 (next)** — [ ] The second batch. Build the 16 missing artifacts per [`docs/design/STACK_IN_A_BOX_PLAN.md`](docs/design/STACK_IN_A_BOX_PLAN.md) §9 (run.sh, requirements.txt, config.example.yml, dbt models, dlt smoke pipeline for NYC 311, semantic-layer YAML, agent YAML, systemd units, portal generators, helper scripts) + the `make rip-out-smoke-test` target. Estimated 10-14 hours. Unblocks a real end-to-end install. Removes the CLAUDE.md §1 "Current install state" caveat as part of its housekeeping. **Inherited contract — must honor (see [`FLOW_DRY_RUN_FINDINGS.md`](docs/design/FLOW_DRY_RUN_FINDINGS.md) F6):** gold model named exactly `main_gold.fct_smoke_test`, admin table `main_admin.fct_pipeline_run_raw`; DuckDB at `data/stack.duckdb`; `config.example.yml` + `dbt/profiles.example.yml` may use ONLY `{{PROJECT_NAME}}` / `{{DUCKDB_PATH}}` tokens (any other `{{...}}` trips 05's unsubstituted-token die); `nginx/stack-in-a-box.conf` must hardcode `/var/www/stack-in-a-box` (07 copies verbatim, no substitution); `run.sh manual` must populate those two tables and deploy the 5 portal pages to `/var/www/stack-in-a-box/`. **Also (F5-b):** if Plan 2 modifies any of scripts 00-03, note that partial-install users must `bootstrap.sh --force` or clear `scratch/checkpoints/`. **F9-F18 deferrals to fold in:** (F10-c) `run.sh` or a new pre-smoke step should run `oxy validate` so a malformed agent/view/topic YAML fails an automated gate instead of only surfacing at the manual browser step; (F12-b) `run.sh` needs DuckDB-lock-aware retry/skip + orphaned-run cleanup (handoff §10 item 2), and consider enabling the systemd timers *after* the first smoke run rather than in step 08; (F14-a) ship a `TEARDOWN.md` (or `make teardown`) in the doc batch — re-install is idempotent but there's no documented clean-slate path; (F9-b) add an "export https_proxy if behind a proxy" hint to `00_preflight.sh`'s network-failure message.
